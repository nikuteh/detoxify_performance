...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "For toxic comments, find each parent comment in a full comments "
            "CSV and plot the parent comments' post_number distribution."
        )
    )
    parser.add_argument(
        "toxic_csv",
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "CSV containing all comments, a comment id column, and post_number "
            "for each comment."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--summary-output",
        type=Path,
        help="Optional output CSV containing counts and percentages by post_number.",
    )
    parser.add_argument(
        "--parent-id-column",
        default="parent_id",
...
def load_toxic_parent_ids(toxic_csv, parent_id_column):
    ...
    if toxic.empty:
        raise SystemExit("No usable t1_ parent ids found in toxic comments CSV")
...
def summarize_parent_post_numbers(
    ...
    matched_toxic_comments = matched[post_number_column].notna().sum()
    plotted_toxic_comments = len(matched_parents)
    ...
    summary["percent_of_all_toxic_comments"] = (
        summary["toxic_comment_count"] / total_toxic_comments * 100
    )

    return summary, {
        "total_toxic_comments": total_toxic_comments,
        "matched_toxic_comments": int(matched_toxic_comments),
...
def plot_histogram(summary, post_number_column, output_png, title, counts):
    ...
    ax.set_ylabel("Percent of toxic comments with matched parents")
...
def main():
    ...
    summary, counts = summarize_parent_post_numbers(
        toxic,
        parent_lookup,
        args.post_number_column,
    ...
    title = args.title or (
        f"{args.toxic_csv.stem}: Parent Post Numbers for Toxic Comments"
    )
    ...
    print(f"Toxic comments with t1_ parent ids: {counts['total_toxic_comments']:,}")
